...
### User Story 6 — 广告投放计划创建与管理 (Priority: P2)

投放优化师小王需要在新平台上创建一条全新的广告投放计划。他在左侧对话面板
或侧边栏点击「广告投放」，填写投放参数（平台、计划名称、日预算、出价、
性别与年龄定向、兴趣标签），系统校验参数后提交到广告平台，经审核确认后
计划正式上线投放，初始数据（消耗/ROI/CTR/CVR）在投放后由平台回传。

**Why this priority**: 广告投放是数字营销服务的起点，新建计划是日常高频操作。
虽然优化已有计划是核心价值（US1），但没有新建投放的能力则无法覆盖从零开始
的客户场景。

**Independent Test**: 独立于优化和内容生产，单独点击广告投放标签 → 填写表单 →
提交 → 确认 → 计划在 Dashboard 中出现。

**Acceptance Scenarios**:

1. **Given** 用户在侧边栏点击「广告投放」，**When** 填写完整表单（平台/名称/预算/出价/定向）
并点击提交，**Then** 系统校验参数合规（预算>=500、出价 5-200），Trace Board 展示
4 节点流式动画（参数配置→平台审核→执行操作→报告生成），审批卡片弹出等待确认。

2. **Given** 审批卡片已弹出，**When** 用户点击「确认执行」，**Then** 计划正式创建并上线，
Dashboard 投放计划列表中出现新计划（状态: active, 初始消耗/ROI/CTR/CVR 均为 0）。

3. **Given** 用户填写的参数不合规（预算<500 或出价超出范围），**When** 点击提交，
**Then** 系统拒绝提交并提示具体错误原因。

4. **Given** 用户点击「取消」或拒绝确认，**When** 审批卡片关闭，**Then** 不创建计划，
表单参数保留以便修改后重新提交。
...
- Q: WebSocket 实时推送 Agent 思考内容的粒度？ → A: 按步骤推送（每个 Agent 节点完成时推送一条消息，含节点名、状态、摘要文本）
- Q: 用户认证与多租户？ → A: 暂不需要（Demo 单用户模式）
- Q: Chroma RAG 数据范围？ → A: 历史优化记录（计划 ID、调整参数、效果）
- Q: Celery 任务取消机制？ → A: 软取消（设置取消标志，Worker 定期检查并优雅退出）
- Q: 部署环境配置？ → A: 单一 `docker-compose.yml`（dev 即 prod）
